...
for _ in range(5):
    ...
    String_Temp=str(input())
    String_Input.append(String_Temp)
    String_Len.append(len(String_Temp))

[ print( String_Input[j][i], end="" ) for i in range( max( String_Len ) ) for j in range(5) if i < String_Len[j] ]

# for i in range(max(String_Len)):

#     for j in range(5):

#         if i < String_Len[j]: print(String_Input[j][i],end="")
...
# I=[];L=[]
# for _ in range(5):T=str(input());I.append(T);L.append(len(T))
# [print(I[j][i],end="")for i in range(max(L))for j in range(5)if i<L[j]]
