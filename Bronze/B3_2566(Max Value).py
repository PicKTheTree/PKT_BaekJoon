...
for _ in range(9):
    String_value.append( list( map( int, input().split() ) ) )

for i in range(9):
        
        Max_value_in_row = max( String_value[i] )

        if Max_value < Max_value_in_row :

            Max_value = Max_value_in_row
            Max_index_x = i + 1
            Max_index_y = String_value[i].index(Max_value_in_row) + 1

print( Max_value )
print( Max_index_x, Max_index_y )
