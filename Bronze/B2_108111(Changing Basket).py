...
# 그 범위 안에서 기준이 될 바구니를 선택한다. 도현이가 선택한 바구니의 범위가 begin, end이고, 기준이 되는 바구니를 mid라고 했을 때, begin, begin+1, ..., mid-1, mid, mid+1, ..., end-1, end 순서로 되어있는 바구니의 순서를 mid, mid+1, ..., end-1, end, begin, begin+1, ..., mid-1로 바꾸게 된다.
...
for case in range( Basket_turn_cnt ) :
    ...
    Min, Max, Mid = map( int, input().split() )
    Basket_string = Basket_string[ : Min - 1 ] + Basket_string[ Mid - 1 : Max ] + Basket_string[ Min - 1 : Mid - 1 ] + Basket_string[ Max : ]
