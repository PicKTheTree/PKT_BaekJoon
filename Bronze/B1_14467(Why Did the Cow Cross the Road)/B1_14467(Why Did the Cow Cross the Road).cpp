...
int main()
{
    ...
    // 소의 번호(cowList)를 벡터로 선언하고 초기값을 -1로 둠
    vector<int> cowList; 
    cowList.assign(101, -1);

    int cowNum, cowStatus; 
    int result = 0; 
    while(N--)
    {
        cin >> cowNum >> cowStatus;

        // 입력된 cowStatus와 다른 상태라면 상태 업데이트
        if (cowList[cowNum] != cowStatus)
        {
            // 초기값 상태였다면, result 증가 x
            if (cowList[cowNum] == -1) {cowList[cowNum] = cowStatus;}

            // 아니라면, result 증가 O
            else {cowList[cowNum] = cowStatus; result++;}
        }
    }
    cout << result;
}